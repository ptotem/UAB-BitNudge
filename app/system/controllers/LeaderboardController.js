...
var LeaderboardsController={
  ...
  getUserLeaderboard:function(req,res){
    if(req.query.month)
      LeaderboardsModel.MonthLeaderboard.getLeaderboardOfMonth(req.params.orgId,req.params.teamId,"playerRanks",{},{path:'playerRanks.player'},function(err,obj){
        if(err) res.send(err);
        else res.send(obj);
      });
    else if(req.query.quarter)
  ...
  getOrganizationLeaderboard:function(req,res){
    if(req.query.month)
      LeaderboardsModel.MonthLeaderboard.getLeaderboardOfMonth(req.params.orgId,req.params.teamId,"teamRanks",{},{path:'teamRanks.team'},function(err,obj){
        if(err) res.send(err);
        else res.send(obj);
      });
    else if(req.query.quarter)
